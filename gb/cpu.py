...
class CPU:
    ...
    def _exec(self, opcode: int, op_off: int) -> int:
        ...
        if opcode == 0x36:
            v = self._imm8(op_off)
            self._write8(self.regs.get_hl(), v)
            self.pc = (self.pc + inc2) & 0xFFFF
            return 12

        if opcode == 0x34:
            addr = self.regs.get_hl()
            self._write8(addr, self._inc8(self._read8(addr)))
            self.pc = (self.pc + inc1) & 0xFFFF
            return 12

        if opcode == 0x35:
            addr = self.regs.get_hl()
            self._write8(addr, self._dec8(self._read8(addr)))
            self.pc = (self.pc + inc1) & 0xFFFF
            return 12
        ...
        if opcode in (0xC3, 0xC2, 0xCA, 0xD2, 0xDA):
            addr = self._imm16(op_off)
            if opcode == 0xC3:
                self.pc = addr
                return 16
        ...
        if opcode in (0xC7, 0xCF, 0xD7, 0xDF, 0xE7, 0xEF, 0xF7, 0xFF):
            vec = opcode & 0x38
            self.push_u16((self.pc + inc1) & 0xFFFF)
            self.pc = vec
            return 16
